...
printr "[!] Enter y when FakeFS finishes creating. "
read fakefsdone
sleep 1
...
if [ "$fakefsdone" = "y" ]; then
    ...
    printr "[!] Enter y when the device finishes jailbreaking "
    read finishedjb1
    ...
    if [ "$finishedjb1" = "y" ]; then
        ...
        printg
        printg "[*] Enter y when you finish installing openssh"
        ...
        printr "[!] Exit code: 104"
        exit 104
    fi

else
    printr "[!] Exit code: 103"
    exit 103
fi
